...
class DistractionDetector:
    def __init__(self, fps=30):
        ...
        self.distraction_patterns = {
            ...
            'eating': {'score': 0, 'threshold': 0.6},
            'reaching': {'score': 0, 'threshold': 0.5},
            'looking_away': {'score': 0, 'threshold': 0.6},
            'drowsy': {'score': 0, 'threshold': 0.7}
        }
        
        # === 時間閾值 ===
        self.PHONE_USE_TIME = 2.0      # 使用手機
        self.FACE_AWAY_TIME = 2.0      # 臉部離開
        self.LOOKING_AWAY_TIME = 3.0   # 視線偏離
        self.REACHING_TIME = 1.0       # 伸手拿東西
    ...
    def analyze_frame(self, frame, frame_count):
            ...
            if hand_results.multi_hand_landmarks:
                for hand_idx, hand_landmarks in enumerate(hand_results.multi_hand_landmarks):
                    ...
                    if self.is_dangerous_reach(hand_landmarks, pose_results):
                        events.append("A08: 危險的伸手動作")
                        self.stats['dangerous_reaches'] += 1
        ...
        if self.distraction_level > 70:
            events.append(f"警告: 高度分心狀態 (分心度: {self.distraction_level:.0f}%)")
        elif self.distraction_level > 50:
            events.append(f"提醒: 中度分心 (分心度: {self.distraction_level:.0f}%)")
    ...
    def extract_features(self, face_results, hand_results, pose_results, frame_shape):
        ...
        if face_results.multi_face_landmarks:
            ...
            features.extend(face_features[:30])  # 限制為30個特徵
        else:
        ...
        if hand_results.multi_hand_landmarks:
            ...
            hand_features = []
            for lm in hand_lms[:5]:  # 使用前5個關鍵點
                hand_features.extend([lm.x, lm.y])
            features.extend(hand_features[:10])
        else:
        ...
        if pose_results.pose_landmarks:
            ...
            # 肩膀和手肘位置
            shoulder_left = pose_lms[11]
            shoulder_right = pose_lms[12]
            features.extend([
    ...
    def estimate_gaze(self, face_landmarks):
        ...
        # 使用眼睛和鼻子的相對位置估算視線
        left_eye = landmarks[33]  # 左眼內角
        right_eye = landmarks[133]  # 右眼內角
    ...
    def is_looking_at_road(self, gaze_direction):
        """判斷是否在看路"""
        # 簡化判斷：視線在中央區域
        return abs(gaze_direction['x']) < 0.1 and abs(gaze_direction['y']) < 0.1
    ...
    def is_dangerous_reach(self, hand_landmarks, pose_results):
        ...
        hand_position = hand_landmarks.landmark[9]  # 手掌中心
        pose_landmarks = pose_results.pose_landmarks.landmark
        
        # 檢查手是否遠離身體中心
        shoulder_center_x = (pose_landmarks[11].x + pose_landmarks[12].x) / 2
        shoulder_center_y = (pose_landmarks[11].y + pose_landmarks[12].y) / 2
        
        distance_from_center = np.sqrt(
            (hand_position.x - shoulder_center_x) ** 2 +
            (hand_position.y - shoulder_center_y) ** 2
        )
        
        # 如果手離身體中心太遠，視為危險伸手
        return distance_from_center > 0.4
    ...
    def visualize_results(self, frame, features):
        ...
        stats_text = f"Phone: {self.stats['phone_uses']} | " \
                    ...
                    f"Reaches: {self.stats['dangerous_reaches']}"
