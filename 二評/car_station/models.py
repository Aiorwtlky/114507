...
class EventRule(db.Model):
    ...
    rule_id = db.Column(db.String(10), primary_key=True, comment='規則ID: A01, A02...')
    category = db.Column(db.String(20), nullable=False, comment='類別: inside/outside')
    event_name = db.Column(db.String(100), nullable=False, comment='事件名稱')
    description = db.Column(db.String(255), nullable=False, comment='事件描述')
    deduction_points = db.Column(db.Integer, nullable=False, comment='扣分')
    threshold_seconds = db.Column(db.Float, comment='時間閾值（秒）')
    severity = db.Column(db.Integer, default=2, comment='嚴重程度 1-3')
    is_active = db.Column(db.Boolean, default=True, comment='是否啟用')
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    ...
    def init_default_rules(cls):
        ...
        default_rules = [
            # 內鏡頭
            cls(rule_id='A01', category='inside', event_name='重度疲勞',
                description='閉眼5秒以上', deduction_points=-40, 
                threshold_seconds=5.0, severity=3),
            cls(rule_id='A02', category='inside', event_name='中度疲勞',
                description='閉眼3-5秒', deduction_points=-30,
                threshold_seconds=3.0, severity=2),
            cls(rule_id='A03', category='inside', event_name='使用手機',
                description='手持手機', deduction_points=-15,
                threshold_seconds=3.0, severity=2),
            cls(rule_id='A04', category='inside', event_name='注意力分散',
                description='臉部離開畫面', deduction_points=-40,
                threshold_seconds=5.0, severity=3),
            # 外鏡頭
            cls(rule_id='B01', category='outside', event_name='切換車道未打燈',
                description='變換車道未打方向燈', deduction_points=-15,
                threshold_seconds=None, severity=2),
            cls(rule_id='B02', category='outside', event_name='轉彎未打燈',
                description='轉彎未打方向燈', deduction_points=-15,
                threshold_seconds=None, severity=2),
            cls(rule_id='B03', category='outside', event_name='未保持安全距離',
                description='前車距離過近', deduction_points=-15,
                threshold_seconds=2.0, severity=2)
        ]
        
        for rule in default_rules:
            existing = cls.query.get(rule.rule_id)
            if not existing:
                db.session.add(rule)
...
class ScoringInterval(db.Model):
    """15分鐘區間評分表"""
    __tablename__ = 'scoring_interval'
    
    id = db.Column(db.Integer, primary_key=True)
    trip_id = db.Column(db.Integer, db.ForeignKey('trip.id'), nullable=False)
    interval_number = db.Column(db.Integer, nullable=False, comment='區間序號')
    interval_start = db.Column(db.DateTime, nullable=False, comment='區間開始時間')
    interval_end = db.Column(db.DateTime, comment='區間結束時間')
    
    # 評分
    inside_score = db.Column(db.Integer, default=100, comment='車內評分')
    outside_score = db.Column(db.Integer, default=100, comment='車外評分')
    
    # 事件統計
    inside_event_count = db.Column(db.Integer, default=0, comment='車內事件數')
    outside_event_count = db.Column(db.Integer, default=0, comment='車外事件數')
    
    # 狀態
    is_failed = db.Column(db.Boolean, default=False, comment='是否不及格(<60分)')
    is_completed = db.Column(db.Boolean, default=False, comment='是否已結算')
    
    # 詳細事件列表 (JSON)
    inside_events = db.Column(db.Text, comment='車內事件詳情 JSON')
    outside_events = db.Column(db.Text, comment='車外事件詳情 JSON')
    
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    trip = db.relationship('Trip', backref='scoring_intervals')
    ...
    def to_dict(self):
        return {
            ...
            'inside_score': self.inside_score,
            'outside_score': self.outside_score,
            'inside_event_count': self.inside_event_count,
    ...
    @classmethod
    def create_new_interval(cls, trip_id, interval_number, start_time):
        """建立新的評分區間"""
        interval = cls(
            trip_id=trip_id,
...
class AIDetectionResult(db.Model):
    ...
    camera_type = db.Column(db.String(10), nullable=False, comment='inside/outside')
    detection_type = db.Column(db.String(50), nullable=False, comment='偵測類型')
    
    # 偵測詳情 (JSON)
    detection_data = db.Column(db.Text, comment='偵測詳細資料 JSON')
    
    # 關聯的事件 (如果有)
    event_id = db.Column(db.Integer, db.ForeignKey('event_log.id'))
    
    trip = db.relationship('Trip', backref='ai_detections')
    event = db.relationship('EventLog', backref='ai_detection')
    ...
    def to_dict(self):
        return {
            ...
            'detection_data': json.loads(self.detection_data) if self.detection_data else None
        }