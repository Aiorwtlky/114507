...
<body>
  ...
  <script>
    ...
    // 狀態
    let W = 0, H = 0;     // 畫布 CSS 尺寸
    let CW = 0, CH = 0;   // 畫布真實像素
    ...
    // 動畫狀態
    let startTime = 0;
    const DURATION = 680; // 首次載入動畫時長（ms）

    // 將資料包成物件，含每一欄的座標（繪製後會填入）
    const state = RAW.map((d, i) => ({
      i,
      name: d.name,
      value: d.value,
      x: 0,
    ...
    function layout() {
      ...
      // 平均線位置
      const yAvg = toY(AVG, plotH);
      avgTag.style.top = `${yAvg + PAD.top}px`;
    ...
    function text(txt, x, y, opt = {}) {
      ...
      ctx.save();
      ctx.font = `${weight} ${size}px system-ui,Segoe UI,Arial`;
      ctx.fillStyle = color;
      ctx.textAlign = align;
    ...
    function draw(progress = 1) {
      ...
      for (let i = 0; i <= rows; i++) {
        const y = plotT + (plotB - plotT) * (i / rows);
        line(plotL, y, plotR, y, { color: '#e5e7eb', width: 1 });
      }

      // 平均線（虛線）
      const yAvg = valueToY(AVG);
      line(plotL, yAvg, plotR, yAvg, { color: '#60a5fa', width: 1.5, dash: [4, 6] });
      ...
      for (const d of state) {
        const x = d.x;
        const y = Util.lerp(plotB, d.y, Util.ease(progress));
        const w = d.w;
        const h = Math.max(0, plotB - y);

        const isHot = d.value >= 120; // 高於 120 以熱色呈現
        const color = isHot ? gHot : gNorm;

        // 陰影
        ctx.save();
        ctx.shadowColor = 'rgba(2,6,23,.12)';
        ctx.shadowBlur = 12;
        ctx.shadowOffsetY = 6;
        ...
        // 縣市名稱（旋轉 40 度）
        ctx.save();
        ctx.translate(x + w/2, plotB + 8);
        ctx.rotate(-Math.PI / 5);
        text(d.name, 0, 0, { size: 12, color: '#475569', align: 'right', base: 'top', weight: '600' });
        ctx.restore();
        ...
        if (hoverIndex === d.i) {
          ctx.save();
          ctx.globalAlpha = .12;
          fillRect(x - 4, plotT, w + 8, plotB - plotT, '#3b82f6');
          ctx.restore();
        }
      }
      ...
      // Y 軸刻度（0、50、100、150、200）
      const ticks = [0, 50, 100, 150, 200];
      for (const t of ticks) {
        const yy = valueToY(t);
        text(String(t), plotL - 8, yy, { size: 12, color: '#64748b', align: 'right', base: 'middle', weight: '600' });
      }

      // 更新平均線標籤位置
      avgTag.style.left = `${plotR - 72}px`;
    ...
    function animate(ts) {
      ...
      const t = Util.clamp((ts - startTime) / DURATION, 0, 1);
      draw(t);
      if (t < 1) requestAnimationFrame(animate);
    }

    // --------------------------------------------
    // 滑鼠互動：偵測 hover 欄位 + tooltip
    // --------------------------------------------
    function onMove(ev) {
      ...
      let hit = -1;
      for (const d of state) {
        if (px >= d.x && px <= d.x + d.w && py >= d.y && py <= H - PAD.bottom) {
          hit = d.i; break;
        }
      }
      ...
      if (hoverIndex >= 0) {
        const d = state[hoverIndex];
        tip.innerHTML = `<strong>${d.name}</strong><br/>死亡人數：<b>${Util.num(d.value)}</b>`;
        tip.style.left = `${d.x + d.w/2}px`;
    ...
    selects.forEach(sel => sel.addEventListener('change', () => {
      ...
      AVG = y === '114' ? 57.0 : (y === '113' ? 59.5 : 61.2);
      layout();
      startTime = 0;
